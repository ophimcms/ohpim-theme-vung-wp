<?php
class WG_oPhim_Sidebar extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        <?php if ($query->have_posts()) :
            switch ($style) {
                case 1:
                    include SIDEBARTEMPLADE.'/type1.php';
                    break;
                case 2:
                    include SIDEBARTEMPLADE.'/type2.php';
                    break;
                case 3:
                    include SIDEBARTEMPLADE.'/type3.php';
                    break;
                default:
                    include THEME_URL.'/templates/slide_home.php';
            }
            endif; ?>
    ...
    function form($instance)
    {
        ...
        <p>
            <label><?php _e('Hiển thị thanh bên', 'ophim') ?></label>
            <br>
            <?php
            $f = array( '1' => __('Kiểu 1', 'ophim'), '2' => __('Kiểu 2', 'ophim'), '3' => __('Kiểu 3', 'ophim'));
            foreach ($f as $x => $n ) { ?>
                <label for="<?php echo $this->get_field_id("style"); ?>_<?php echo $x ?>">
                    <input id="<?php echo $this->get_field_id("style"); ?>_<?php echo $x ?>" class="<?php echo $x ?>" name="<?php echo $this->get_field_name("style"); ?>" type="radio" value="<?php echo $x ?>" <?php if (isset($style)) { checked( $x, $style, true ); } ?> /> <?php echo $n ?>
                </label>
            <?php } ?>
        </p>
...
function register_new_WG_oPhim_Sidebar() {
    register_widget( 'WG_oPhim_Sidebar' );
}
add_action( 'widgets_init', 'register_new_WG_oPhim_Sidebar' );
